...
유치원생(전체)·초등학생(전체) 대상 추천도서 400권을 **서점 추천 200** + **도서관 추천 200** 으로 나눠 보여 주는 단일 웹페이지입니다. (유아 서점/도서관 각 100 + 초등 서점/도서관 각 100) GitHub Pages로 `index.html` 이 게시됩니다.
...
3. `books.json` 의 **`store.유아`, `store.초등` 만** 최신 인기/신간을 반영해 재정렬·교체한다.
   - 각 목록은 **100권 유지**, 엔트리 형식(위 8개 필드)·분야 규칙을 지킨다. 추천이유는 한국어 100자 내외.
